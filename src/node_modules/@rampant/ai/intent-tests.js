/* eslint-disable no-console */
import { start, importProject, db } from '@rampant/ai'
import { white, underline } from 'chalk'
import serial from '@rampant/serial-promise'
import {
    testCount,
    projectSettings,
    cleanupDocs,
} from '@rampant/ai/test-intents.js'

start({ projectId: 'testing' })

const createIntents = () => {
    console.log()
    console.log(`  ${white(underline('Intent Tests'))}`)
    console.log()

    return importProject('testing', projectSettings)
}

const cleanup = () => {
    console.log()
    console.log(white.bold(`    ${testCount} tests run`))
    console.log()

    return Promise.all([
        ...projectSettings.intents.map(({ name }) =>
            db.remove(`projects/testing/intents/${name}`)
        ),
        ...cleanupDocs.map(doc =>
            db.remove(`projects/testing/custom-db/${doc}`)
        ),
    ])
}

serial([createIntents, cleanup], 2000).then(() => process.exit(0))
